...
public final class RoutingTableNetworkProvider: NetworkIdentityProviding {
    ...
    static func readGatewayIdentity() -> NetworkIdentity? {
    ...
    private static func readDefaultGatewayIP() -> in_addr_t? {
        ...
        while offset + MemoryLayout<rt_msghdr>.size <= needed {
            ...
            guard msgLen >= MemoryLayout<rt_msghdr>.size, offset + msgLen <= needed else { break }
            ...
            if (rtm.rtm_flags & RTF_GATEWAY) != 0 {
                ...
                for bit in 0..<8 {
                    ...
                    guard saOff + 2 <= offset + msgLen else { break }
            ...
            for bit in 0..<8 {
                ...
                if bit == 0, sa.sa_family == UInt8(AF_INET) { // RTA_DST
                    ...
                    if alen == 6 {
                        ...
                        if macStart + alen <= offset + msgLen {
                            macStr = (0..<alen)
